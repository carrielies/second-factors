...
function checkECDSASignature(data, cert, signature) {
    ...
    console.log(convertCertToPEM(cert))
    var asn1 = pkijs.org.pkijs.fromBER(str2ab(cert.toString("binary")));
    var cert_simpl = new pkijs.org.pkijs.simpl.CERT({ schema: asn1.result });
    let arrayBuffer = cert_simpl.subjectPublicKeyInfo.subjectPublicKey.value_before_decode
    let a = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)));
    let pubKey = new Buffer(a, 'base64');
    console.log(a)
    ...
    MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEP3fU3HzTfsSK1FVPjshEZ7TISqBg\n
    2/4wIL3McyUgbFP/L0ab0Q6ILr3oUiY6OmSoX+dHc0poaxFU3tRKYFUpTQ==\n
    -----END PUBLIC KEY-----\n`
